...
public class QuestController : MonoBehaviour {
	...
	public GameObject quest_guardianOfTheVillage_guardianTrigger;
			...
			foreach (FileInfo f in info) {
				...
				case "GuardianOfTheSacredForest":
					...
					if (QDM.QD.QI.qCompleted)  {
						...
						bool sacredBookFound = false;
						for (int i=0; i<QDM.QD.GI.Count; i++)  {
							if (QDM.QD.GI[i].gTarget == "sacredBook")  {
								if (QDM.QD.GI[i].gCurrentAmount != 0)  {
									sacredBookFound = true;
								}
							}
						}
						if (sacredBookFound)  {
							Destroy (quest_guardianOfTheVillage_guardianTrigger);
							quest_guardianOfTheSacredForest_cursedMist.gameObject.SetActive (true);
						}
